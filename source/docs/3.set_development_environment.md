...
(1) Start the robot and connect it to the VNC. For more details about the connection, please refer to "[2. Set Development Environment -\>2.1 VNC Installation and Connection]()".
...
(1) Turn on the device. Follow the instructions in " <a href="https://docs.hiwonder.com/projects/TurboPi/en/latest/docs/1.getting_ready.html#remote-desktop-installation-and-connection" target="_blank"> 2.1 VNC Installation and Connection</a>" to connect to the computer via VNC.
...
Obtain the unique container ID based on "[2.4.2 Docker Command -> Check Container](#anchor_4_2_1)". Enter the command "docker exec -it -u ubuntu -w /home/ubuntu adb8 /bin/bash" to access the container that contains the function programs.
